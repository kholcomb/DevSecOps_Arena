...
find domains -maxdepth 1 -mindepth 1 -type d ! -name '_*' -exec basename {} \; | sed 's/^/  - /'
echo ""
echo "To start playing:"
echo "  ./play.sh                        # Interactive domain selection"
echo "  ./play.sh --domain kubernetes    # Pre-select specific domain"
echo "  ./play.sh --help                 # Show all options"
echo ""
