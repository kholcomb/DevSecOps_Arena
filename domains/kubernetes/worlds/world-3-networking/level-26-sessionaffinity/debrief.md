...
kubectl get service session-service -n arena -o yaml | grep -A3 sessionAffinity
```
...
kubectl exec client -n arena -- sh -c 'for i in 1 2 3 4 5; do wget -q -O- http://session-service; echo; done'
```
...
kubectl run client2 -n arena --image=busybox:1.36 -- sleep 3600

# Request from client2 (different source IP)
kubectl exec client2 -n arena -- wget -q -O- http://session-service
```
...
kubectl delete pod client -n arena
kubectl run client -n arena --image=busybox:1.36 --command -- sh -c 'while true; do wget -q -O- http://session-service 2>&1; sleep 2; done'
```
...
kubectl top pods -n arena -l app=session-app
```
