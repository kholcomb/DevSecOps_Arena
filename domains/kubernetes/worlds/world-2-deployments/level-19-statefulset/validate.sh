...
if kubectl get statefulset database -n arena &>/dev/null; then
    READY=$(kubectl get statefulset database -n arena -o jsonpath='{.status.readyReplicas}' 2>/dev/null)
    DESIRED=$(kubectl get statefulset database -n arena -o jsonpath='{.spec.replicas}' 2>/dev/null)
    ...
    if [ "$READY" = "$DESIRED" ]; then
        ...
        POD_NAMES=$(kubectl get pods -n arena -l app=database -o jsonpath='{.items[*].metadata.name}' 2>/dev/null)
    ...
    if kubectl get deployment database -n arena &>/dev/null; then
