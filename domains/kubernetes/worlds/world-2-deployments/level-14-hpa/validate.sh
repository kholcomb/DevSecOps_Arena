...
HPA_STATUS=$(kubectl get hpa web-backend-hpa -n arena -o jsonpath='{.status.conditions[?(@.type=="ScalingActive")].status}' 2>/dev/null)
...
if [ "$HPA_STATUS" = "True" ]; then
    ...
    if kubectl top pods -n arena &>/dev/null; then
