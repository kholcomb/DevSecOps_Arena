...
READY_REPLICAS=$(kubectl get deployment web-app -n arena -o jsonpath='{.status.readyReplicas}' 2>/dev/null)
DESIRED_REPLICAS=$(kubectl get deployment web-app -n arena -o jsonpath='{.spec.replicas}' 2>/dev/null)

# Also check that we're not on the broken image
CURRENT_IMAGE=$(kubectl get deployment web-app -n arena -o jsonpath='{.spec.template.spec.containers[0].image}' 2>/dev/null)
