...
MIN_AVAILABLE=$(kubectl get pdb db-proxy-pdb -n arena -o jsonpath='{.spec.minAvailable}' 2>/dev/null)
REPLICAS=$(kubectl get deployment database-proxy -n arena -o jsonpath='{.spec.replicas}' 2>/dev/null)

# Check if PDB allows evictions
ALLOWED=$(kubectl get pdb db-proxy-pdb -n arena -o jsonpath='{.status.disruptionsAllowed}' 2>/dev/null)
