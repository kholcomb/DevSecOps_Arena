...
if kubectl get deployment web-app -n arena &>/dev/null; then
    READY=$(kubectl get deployment web-app -n arena -o jsonpath='{.status.readyReplicas}' 2>/dev/null)
    DESIRED=$(kubectl get deployment web-app -n arena -o jsonpath='{.spec.replicas}' 2>/dev/null)
    ...
    if [ "$READY" = "$DESIRED" ]; then
        ...
        RS_COUNT=$(kubectl get replicaset -n arena -l app=webapp -o name 2>/dev/null | wc -l | tr -d ' ')
    ...
    if kubectl get replicaset web-app-rs -n arena &>/dev/null; then
