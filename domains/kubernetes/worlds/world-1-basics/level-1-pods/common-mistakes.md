...
**What players try:**
```bash
kubectl logs nginx-broken -n arena
```
...
**Correct approach:**
```bash
kubectl logs nginx-broken --previous -n arena
```
...
**What players try:**
```bash
kubectl edit pod nginx-broken -n arena
...
kubectl apply -f solution.yaml -n arena
```
...
kubectl describe pod nginx-broken -n arena | grep -A 5 "State:"

# Or look at container status
kubectl get pod nginx-broken -n arena -o jsonpath='{.status.containerStatuses[*].name}'
```
...
**Correct approach:**
```bash
kubectl describe pod nginx-broken -n arena | grep "Exit Code"
```
...
# Edit my-fix.yaml
kubectl apply -f my-fix.yaml -n arena
```
...
kubectl describe pod nginx-broken -n arena | grep -A 20 Events
```
...
kubectl wait --for=condition=ready pod/nginx-broken -n arena --timeout=60s
...
**Key Learning:**
Namespaces isolate resources. Always use `-n arena` or set it as default.
...
# 8. Verify fix
kubectl get pod nginx-broken -n arena -w
...
5. **Specify namespace** - use `-n arena` or set default context
